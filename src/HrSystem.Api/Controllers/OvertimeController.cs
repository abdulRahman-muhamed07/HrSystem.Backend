...
public sealed class OvertimeController(OvertimeHandler handler) : ControllerBase
{
    ...
    [HttpGet("pending")]
    public Task<IReadOnlyCollection<OvertimeDto>> GetPending(CancellationToken ct) => handler.GetPendingAsync(ct);

    [HttpPost]
    public async Task<ActionResult<int>> Create(CreateOvertimeRequest request, CancellationToken ct)
    {
        var id = await handler.CreateAsync(request, ct);
        return Created($"api/overtime/{id}", id);
    }
