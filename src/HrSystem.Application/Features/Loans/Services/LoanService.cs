...
public sealed class LoanService(
    ...
    IAuditService audit,
    ICurrentUser currentUser) : ILoanService
{
    ...
    public async Task<int> CreateAsync(CreateLoanRequest request, CancellationToken ct)
    {
        if (!IsHrOrAdmin() && currentUser.EmployeeId != request.EmployeeId)
            throw new BusinessRuleException("You can only create a loan request for yourself.");
        ...
        await loans.AddAsync(entity, ct);
        await unitOfWork.SaveChangesAsync(ct);
        await audit.WriteAsync("Create", nameof(EmployeeLoan), entity.Id.ToString(), null, ct);
        return entity.Id;
    }

    public async Task<IReadOnlyCollection<LoanDto>> GetPendingAsync(CancellationToken ct)
    {
        if (!IsHrOrAdmin())
            throw new BusinessRuleException("Only HR or Admin users can view pending loan requests.");

        return await loans.QueryAsync(
            l => new LoanDto(l.Id, l.EmployeeId, l.Employee!.FullName, l.Amount, l.Installments, l.MonthlyDeduction, l.RemainingAmount, l.Reason, l.Status),
    ...
    {
        if (!IsHrOrAdmin())
            throw new BusinessRuleException("Only HR or Admin users can decide loan requests.");
        ...
        await unitOfWork.SaveChangesAsync(ct);
        await audit.WriteAsync(approve ? "Approve" : "Reject", nameof(EmployeeLoan), id.ToString(), null, ct);
    }
}
