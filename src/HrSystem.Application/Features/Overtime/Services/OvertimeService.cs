...
public sealed class OvertimeService(
    ...
    IAuditService audit,
    ICurrentUser currentUser) : IOvertimeService
{
    ...
    {
        if (!IsHrOrAdmin() && currentUser.EmployeeId != request.EmployeeId)
            throw new BusinessRuleException("You can only create an overtime request for yourself.");
        ...
        await overtime.AddAsync(entity, ct);
        await unitOfWork.SaveChangesAsync(ct);
        await audit.WriteAsync("Create", nameof(OvertimeRequest), entity.Id.ToString(), null, ct);
        return entity.Id;
    }

    public async Task<IReadOnlyCollection<OvertimeDto>> GetPendingAsync(CancellationToken ct)
    {
        if (!IsHrOrAdmin())
            throw new BusinessRuleException("Only HR or Admin users can view pending overtime requests.");

        return await overtime.QueryAsync(
            o => new OvertimeDto(o.Id, o.EmployeeId, o.Employee!.FullName, o.Date, o.Hours, o.RateMultiplier, o.Reason, o.Status),
            o => o.Status == OvertimeStatus.Pending,
            0,
            int.MaxValue,
            ct);
    }
    ...
    {
        if (!IsHrOrAdmin())
            throw new BusinessRuleException("Only HR or Admin users can decide overtime requests.");

        var entity = await overtime.GetByIdAsync(id, ct) ?? throw new NotFoundException("Overtime request was not found.");
        if (entity.Status != OvertimeStatus.Pending) throw new BusinessRuleException("Only pending overtime requests can be decided.");
        ...
        await unitOfWork.SaveChangesAsync(ct);
        await audit.WriteAsync(approve ? "Approve" : "Reject", nameof(OvertimeRequest), id.ToString(), null, ct);
    }
}
