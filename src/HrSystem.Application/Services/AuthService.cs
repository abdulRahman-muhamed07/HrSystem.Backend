    ...
    IValidator<RegisterRequest> registerValidator) : IAuthService
{
    public async Task<LoginResponse?> LoginAsync(LoginRequest request, CancellationToken ct)
    {
        ...
        var user = (await users.QueryAsync(u => u,
            u => u.Email == normalized && u.IsActive,
            0, 1, ct)).FirstOrDefault();
        ...
        return await unitOfWork.ExecuteInTransactionAsync(async token =>
        {
            ...
            return new LoginResponse(accessToken, accessExpiresAt,
                refresh.RawToken, refresh.ExpiresAt,
                user.Id, user.FullName, user.Role, user.EmployeeId);
        }, ct);
    }
    ...
    public async Task<LoginResponse?> RefreshAsync(RefreshTokenRequest request, CancellationToken ct)
    {
        ...
        var current = (await refreshTokens.QueryAsync(r => r,
            r => r.TokenHash == hash && r.RevokedAt == null && r.ExpiresAt > DateTime.UtcNow,
            0, 1, ct)).FirstOrDefault();
    ...
    public async Task LogoutAsync(string jti, DateTimeOffset expiresAt, string? refreshToken, CancellationToken ct)
    {
        ...
        if (!string.IsNullOrWhiteSpace(refreshToken))
        {
            ...
            var stored = (await refreshTokens.QueryAsync(r => r,
                r => r.TokenHash == hash && r.RevokedAt == null,
                0, 1, ct)).FirstOrDefault();
            stored?.Revoke();
